...
def get_trace_result_analysis(trace_result, gpu=False, layerwise_kernel=False):
    ...
    assert len(batch_evaluation_times) == len(preprocess_times) == len(predict_times) == len(postprocess_times) 
...
def find_kernels_to_optimize(kernel_times, print_kernels=False, save_fig=False, fig_path='./kernel_performance_metrics.png'):
    metrics = {} 
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process trace result including GPU kernel information.')
    parser.add_argument('--trace_result_dir', type=str, default='.', help='Directory of the trace result file')
    parser.add_argument('--trace_result_path', type=str, default='trace_result.txt', help='Path to the trace result file')
    parser.add_argument('--top_k', type=int, default=5, help='Top k time consuming layers')
    parser.add_argument('--gpu_trace', type=str, default='true', choices=['true', 'false'], help='Whether the trace result includes GPU kernel information')
    parser.add_argument('--layerwise_kernel', type=str, default='true', choices=['true', 'false'], help='Whether to analyze kernel time for each layer')
    parser.add_argument('--find_kernels_to_optimize', type=str, default='true', choices=['true', 'false'], help='Whether to find kernels to optimize')
    parser.add_argument('--save_fig', type=str, default='false', choices=['true', 'false'], help='Whether to save the figure of kernel performance metrics')
    parser.add_argument('--fig_path', type=str, default='./kernel_performance_metrics.png', help='Path to save the figure of kernel performance metrics')
    parser.add_argument('--average_time_for_layer', type=str, default='true', choices=['true', 'false'], help='Whether to calculate average time for each layer')
    parser.add_argument('--time_consuming_layers_by_depth', type=str, default='true', choices=['true', 'false'], help='Whether to get time consuming layers by depth')
    parser.add_argument('--average_time_for_kernel', type=str, default='false', choices=['true', 'false'], help='Whether to calculate average time for each kernel')
    parser.add_argument('--top_k_avg_time_consuming_kernels', type=str, default='false', choices=['true', 'false'], help='Whether to get top k average time consuming kernels')
    parser.add_argument('--top_k_total_time_consuming_kernels', type=str, default='false', choices=['true', 'false'], help='Whether to get top k total time consuming kernels')
    parser.add_argument('--top_k_total_time_consuming_kernels_with_count', type=str, default='false', choices=['true', 'false'], help='Whether to get top k total time consuming kernels with count')
    
    args = parser.parse_args()

    gpu = args.gpu_trace == 'true'
    layerwise_kernel = args.layerwise_kernel == 'true'

    trace_result = get_trace_result(args.trace_result_dir + '/' + args.trace_result_path)
    
    
    num_batch_evaluations, avg_batch_evaluation_time, avg_preprocess_time, avg_predict_time, avg_postprocess_time, layer_times, kernel_times, kernel_for_layer = get_trace_result_analysis(trace_result, gpu=gpu, layerwise_kernel=layerwise_kernel)
    
    print(f"Number of batch evaluations: {num_batch_evaluations}")
    print(f"Average batch evaluation time: {avg_batch_evaluation_time}")
    print(f"Average preprocess time: {avg_preprocess_time}")
    print(f"Average predict time: {avg_predict_time}")
    print(f"Average postprocess time: {avg_postprocess_time}")

    # ms 
    print(f"Average batch evaluation time: {avg_batch_evaluation_time * 1000} ms")
    print(f"Average preprocess time: {avg_preprocess_time * 1000} ms")
    print(f"Average predict time: {avg_predict_time * 1000} ms")
    print(f"Average postprocess time: {avg_postprocess_time * 1000} ms")

    if args.average_time_for_layer == 'true':
        avg_layer_times = get_avg_time_for_layer(layer_times, print_layer_times=True) 

    if args.time_consuming_layers_by_depth == 'true':
        layer_times_by_depth = get_time_consuming_layers_by_depth(layer_times, top_k_for_print=args.top_k, print_layer_times=True, kernel_for_layer=kernel_for_layer) 

    if args.average_time_for_kernel == 'true':
        avg_kernel_times = get_avg_time_for_kernel(kernel_times, print_kernel_times=True)
    
    if args.top_k_avg_time_consuming_kernels == 'true':
        top_k_kernel_times = get_top_k_avg_time_consuming_kernels(kernel_times, top_k=args.top_k) 
        print(f"Top {args.top_k} average time consuming kernels: ")
        for kernel, time in top_k_kernel_times:
            print(f"{kernel}: {time * 1000} ms")
    
    if args.top_k_total_time_consuming_kernels == 'true':
        top_k_total_kernel_times = get_top_k_total_time_consuming_kernels(kernel_times, top_k=args.top_k)
        print(f"Top {args.top_k} total time consuming kernels: ")
        for kernel, time in top_k_total_kernel_times:
            print(f"{kernel}: {time * 1000} ms")
    
    if args.top_k_total_time_consuming_kernels_with_count == 'true':
        top_k_total_kernel_times = get_top_k_total_time_consuming_kernels_with_count(kernel_times, top_k=args.top_k)
        print(f"\nTop {args.top_k} total time consuming kernels: ")
        for kernel, (total_time, num, avg_time) in top_k_total_kernel_times:
            print(f"{kernel}: {total_time * 1000} ms, num: {num}, avg time: {avg_time * 1000} ms")
    
    if args.find_kernels_to_optimize == 'true':
        save_fig = args.save_fig == 'true'
        target_kernels = find_kernels_to_optimize(kernel_times, print_kernels=False, save_fig=save_fig, fig_path=args.fig_path)
