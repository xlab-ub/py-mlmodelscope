from .models.max_abc import MAXAbstractClass
import inspect
import ast
import pathlib
import sys
...
def create_instance_from_model_manifest_file(task, model_name, security_check=True, config=None, user='default'):
    ...
    # Find the class that inherits from MAXAbstractClass
    abstract_class = MAXAbstractClass
    ...
    if target_class:
        ...
        raise ModuleNotFoundError("No subclass of MAXAbstractClass was found in the module.")
...
def _load(task, model_name, security_check=True, config=None, user='default'):
    ...
    max_abstract_class_instance = create_instance_from_model_manifest_file(task, model_name, security_check, config, user) 
    sys.path.remove(str(model_file_dir))
    return max_abstract_class_instance
