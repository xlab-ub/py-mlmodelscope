...
def create_instance_from_model_manifest_file(task, model_name, providers, security_check=True, config=None, user='default'):
    ...
    abstract_class = MXNetAbstractClass
...
def _load(task, model_name, architecture, security_check=True, config=None, user='default'):
    ...
    mxnet_class_instance = create_instance_from_model_manifest_file(task, model_name, architecture, security_check, config, user)
    sys.path.remove(str(model_file_dir))
    return mxnet_class_instance
