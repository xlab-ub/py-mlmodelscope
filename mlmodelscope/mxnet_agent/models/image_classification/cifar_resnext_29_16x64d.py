...
class MXNet_CIFAR_ResNext_29_16x64D:
  def __init__(self, architecture):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/mxnet/cifar/CIFAR_ResNext_29_16x64d.yml 
    self.inLayer = ['data'] 

    model_symbol_url = "http://s3.amazonaws.com/store.carml.org/models/mxnet/gluoncv/cifar_resnext29_16x64d/model-symbol.json" 
    model_params_url = "http://s3.amazonaws.com/store.carml.org/models/mxnet/gluoncv/cifar_resnext29_16x64d/model-0000.params" 
  ...
  def preprocess(self, input_images):
    for i in range(len(input_images)):
      input_images[i] = np.ascontiguousarray(self.preprocess_cifar(input_images[i]), dtype=np.float32) 
...
def init(architecture):
  return MXNet_CIFAR_ResNext_29_16x64D(architecture) 
