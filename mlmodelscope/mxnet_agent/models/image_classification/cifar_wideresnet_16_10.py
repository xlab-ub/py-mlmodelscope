...
class MXNet_CIFAR_WideResNet_16_10:
  def __init__(self, architecture):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/mxnet/cifar/CIFAR_WideResNet_16_10.yml 
    self.inLayer = ['data'] 

    model_symbol_url = "http://s3.amazonaws.com/store.carml.org/models/mxnet/gluoncv/cifar_wideresnet16_10/model-symbol.json" 
    model_params_url = "http://s3.amazonaws.com/store.carml.org/models/mxnet/gluoncv/cifar_wideresnet16_10/model-0000.params" 
...
def init(architecture):
  return MXNet_CIFAR_WideResNet_16_10(architecture) 
