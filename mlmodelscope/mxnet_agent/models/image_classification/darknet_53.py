...
class MXNet_DarkNet_53:
  def __init__(self, architecture):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/mxnet/darknet/DarkNet_53.yml 
    self.inLayer = ['data'] 

    model_symbol_url = "http://s3.amazonaws.com/store.carml.org/models/mxnet/gluoncv/darknet53/model-symbol.json" 
    model_params_url = "http://s3.amazonaws.com/store.carml.org/models/mxnet/gluoncv/darknet53/model-0000.params" 
...
def init(architecture):
  return MXNet_DarkNet_53(architecture) 
