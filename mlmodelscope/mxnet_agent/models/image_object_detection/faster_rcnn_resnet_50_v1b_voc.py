...
class MXNet_Faster_RCNN_ResNet_50_V1B_VOC:
  def __init__(self, architecture):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_object_detection/mxnet/Faster_RCNN_ResNet_50_v1b_VOC.yml 
    self.inLayer = ['data'] 

    model_symbol_url = "http://s3.amazonaws.com/store.carml.org/models/mxnet/gluoncv/faster_rcnn_resnet50_v1b_voc/model-symbol.json" 
    model_params_url = "http://s3.amazonaws.com/store.carml.org/models/mxnet/gluoncv/faster_rcnn_resnet50_v1b_voc/model-0000.params" 
...
def init(architecture):
  return MXNet_Faster_RCNN_ResNet_50_V1B_VOC(architecture) 
