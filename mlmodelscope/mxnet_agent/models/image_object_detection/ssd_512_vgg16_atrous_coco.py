...
class MXNet_SSD_512_VGG16_Atrous_COCO:
  def __init__(self, architecture):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_object_detection/mxnet/SSD_512_VGG_16_Atrous_COCO.yml 
    self.inLayer = ['data'] 

    model_symbol_url = "http://s3.amazonaws.com/store.carml.org/models/mxnet/gluoncv/ssd_512_vgg16_atrous_coco/model-symbol.json" 
    model_params_url = "http://s3.amazonaws.com/store.carml.org/models/mxnet/gluoncv/ssd_512_vgg16_atrous_coco/model-0000.params" 
...
def init(architecture):
  return MXNet_SSD_512_VGG16_Atrous_COCO(architecture) 
