...
class MXNet_SSD_512_VGG_16_Atrous_VOC:
  def __init__(self, architecture):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_object_detection/mxnet/SSD_512_VGG_16_Atrous_VOC.yml 
    self.inLayer = ['data'] 
...
def init(architecture):
  return MXNet_SSD_512_VGG_16_Atrous_VOC(architecture) 
