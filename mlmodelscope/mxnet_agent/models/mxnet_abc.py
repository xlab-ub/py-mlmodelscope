...
class MXNetAbstractClass(ABC):
    ...
    def model_symbol_and_params_download(self, model_symbol_url: str, model_params_url: str) -> None:
        ...
        model_symbol_path = os.path.join(temp_path, model_name + '/' + model_symbol_url.split('/')[-1]) 
        model_params_path = os.path.join(temp_path, model_name + '/' + model_params_url.split('/')[-1])
