...
def create_instance_from_model_manifest_file(task, model_name, providers, security_check=True, config=None, user='default'):
    ...
    abstract_class = ONNXRuntimeAbstractClass  
    target_class = next((cls for cls in classes if issubclass(cls, abstract_class) and cls != abstract_class), None)
    if target_class:
        return target_class(providers, *(config,) if config else ()) # Create an instance of the found class 
    else:
        raise ModuleNotFoundError("No subclass of ONNXRuntimeAbstractClass was found in the module.") 

def _load(task, model_name, providers, security_check=True, config=None, user='default'):
    model_file_dir = pathlib.Path(__file__).resolve().parent / f'models/{user}/{task}/{model_name}'
    sys.path.append(str(model_file_dir))
    onnxruntime_abstract_class_instance = create_instance_from_model_manifest_file(task, model_name, providers, security_check, config, user) 
    sys.path.remove(str(model_file_dir))
    return onnxruntime_abstract_class_instance
