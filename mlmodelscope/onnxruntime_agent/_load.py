...
def _load(task, model_name, providers, security_check=True):
  try: 
    exec(f'from .models.{task}.' + model_name + ' import init', globals())
    return init(providers)
  except ImportError as e:
    if e.msg.split()[3] == "'init'": 
      return create_instance_from_model_manifest_file(task, model_name, providers, security_check) # Create an instance of the model class 
