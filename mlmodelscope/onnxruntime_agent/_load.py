...
def _load(task, model_name, providers, security_check=True, config=None, user='default'):
  ...
  try: 
    ...
    if e.msg.split()[3] == "'init'": 
      onnxruntime_abstract_class_instance = create_instance_from_model_manifest_file(task, model_name, providers, security_check, config, user) # Create an instance of the model class
      sys.path.remove(model_file_dir)
