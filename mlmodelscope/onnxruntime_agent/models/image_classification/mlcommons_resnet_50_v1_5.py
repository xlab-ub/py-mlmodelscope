...
class ONNXRuntime_MLCommons_ResNet_50_V1_5:
  def __init__(self, providers):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/onnxruntime/mlcommons_inference/MLCommons_ResNet_50_v1.5.yml 
    model_file_url = "https://s3.amazonaws.com/store.carml.org/models/onnxruntime/resnet50_v1.onnx" 
  ...
  def pre_process_vgg(self, img, dims=None, need_transpose=False):
...
def init(providers):
  return ONNXRuntime_MLCommons_ResNet_50_V1_5(providers) 
