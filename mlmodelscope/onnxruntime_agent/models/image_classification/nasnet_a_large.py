...
class ONNXRuntime_NasNet_A_Large:
  def __init__(self, providers):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/onnxruntime/nasnet/NasNet_A_Large.yml 
    model_file_url = "https://s3.amazonaws.com/store.carml.org/models/onnxruntime/nasnetalarge-imagenet.onnx" 
...
def init(providers):
  return ONNXRuntime_NasNet_A_Large(providers) 
