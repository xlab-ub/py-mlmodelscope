...
class ONNXRuntime_ResNext_101_32x4D:
  def __init__(self, providers):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/onnxruntime/resnext/ResNext_101_32x4D.yml 
    model_file_url = "https://s3.amazonaws.com/store.carml.org/models/onnxruntime/resnext101_32x4d-imagenet.onnx" 
...
def init(providers):
  return ONNXRuntime_ResNext_101_32x4D(providers) 
