...
class ONNXRuntime_TorchVision_AlexNet:
  def __init__(self, providers):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/onnxruntime/torchvision/alexnet/TorchVision_AlexNet.yml 
...
def init(providers):
  return ONNXRuntime_TorchVision_AlexNet(providers) 
