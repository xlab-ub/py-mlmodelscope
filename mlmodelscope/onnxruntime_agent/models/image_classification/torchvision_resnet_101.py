...
class ONNXRuntime_TorchVision_ResNet_101:
  def __init__(self, providers):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/onnxruntime/torchvision/resnet/TorchVision_ResNet_101.yml 
    model_file_url = "https://s3.amazonaws.com/store.carml.org/models/onnxruntime/torchvision_resnet101.onnx" 
...
def init(providers):
  return ONNXRuntime_TorchVision_ResNet_101(providers) 
