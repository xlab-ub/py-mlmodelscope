...
class PyTorch_Transformers_CrisperWhisper(PyTorchAbstractClass):
  def __init__(self, config=None):
    self.config = config if config else {} 
    self.processor = AutoProcessor.from_pretrained("nyrahealth/CrisperWhisper")
    self.model = AutoModelForSpeechSeq2Seq.from_pretrained("nyrahealth/CrisperWhisper")
