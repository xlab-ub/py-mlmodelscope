...
class PyTorch_Transformers_Distil_Whisper_Large_v2(PyTorchAbstractClass):
  def __init__(self, config=None):
    self.config = config if config else {} 
    self.processor = AutoProcessor.from_pretrained("distil-whisper/distil-large-v2")
    self.model = AutoModelForSpeechSeq2Seq.from_pretrained("distil-whisper/distil-large-v2")
