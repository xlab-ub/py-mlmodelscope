...
class PyTorch_Transformers_Distil_Whisper_Medium_EN(PyTorchAbstractClass):
  def __init__(self, config=None):
    self.config = config if config else {} 
    self.processor = AutoProcessor.from_pretrained("distil-whisper/distil-medium.en")
    self.model = AutoModelForSpeechSeq2Seq.from_pretrained("distil-whisper/distil-medium.en")
