...
class PyTorch_Transformers_Aldigobbler_Ben2_30m_Test(PyTorchAbstractClass):
    def __init__(self, config=None):
        self.config = config if config else dict()
        device = self.config.pop("_device", "cpu")
        multi_gpu = self.config.pop("_multi_gpu", False)
        model_id = "aldigobbler/ben2_30m_test"
    ...
    def postprocess(self, model_output):
        ...
        for output, original_size in zip(model_output, self.original_sizes):
            # PIL size is (width, height), interpolate needs (height, width)
