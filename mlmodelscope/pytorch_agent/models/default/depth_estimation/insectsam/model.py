...
class PyTorch_Transformers_martintomov_InsectSAM(PyTorchAbstractClass):
    def __init__(self, config=None):
        self.config = config if config else dict()
        device = self.config.pop("_device", "cpu")
        multi_gpu = self.config.pop("_multi_gpu", False)

        self.image_processor = AutoImageProcessor.from_pretrained("martintomov/InsectSAM")
        
        if multi_gpu and device == "cuda":
            self.model = AutoModelForDepthEstimation.from_pretrained("martintomov/InsectSAM", device_map="auto", torch_dtype="auto")
        else:
            self.model = AutoModelForDepthEstimation.from_pretrained("martintomov/InsectSAM")
