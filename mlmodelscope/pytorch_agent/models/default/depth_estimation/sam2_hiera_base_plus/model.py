...
class PyTorch_Transformers_Sam2HieraBasePlus(PyTorchAbstractClass):
    def __init__(self, config=None):
        self.config = config if config else dict()
        device = self.config.pop("_device", "cpu")
        multi_gpu = self.config.pop("_multi_gpu", False)

        self.image_processor = DPTImageProcessor.from_pretrained("merve/sam2-hiera-base-plus")
        
        if multi_gpu and device == "cuda":
            self.model = DPTForDepthEstimation.from_pretrained("merve/sam2-hiera-base-plus", device_map="auto", torch_dtype="auto")
        else:
            self.model = DPTForDepthEstimation.from_pretrained("merve/sam2-hiera-base-plus")
