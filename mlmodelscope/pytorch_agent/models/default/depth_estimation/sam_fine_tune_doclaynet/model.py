...
class PyTorch_Transformers_Mit1208_sam_fine_tune_doclaynet(PyTorchAbstractClass):
    def __init__(self, config=None):
        self.config = config if config else dict()
        device = self.config.pop("_device", "cpu")
        multi_gpu = self.config.pop("_multi_gpu", False)
        model_id = "Mit1208/sam-fine-tune-doclaynet"
    ...
    def postprocess(self, model_output):
        ...
        for output, original_size in zip(model_output, self.original_sizes):
            ...
            # original_size is a tuple of (width, height)
            # interpolate expects size as (height, width)
