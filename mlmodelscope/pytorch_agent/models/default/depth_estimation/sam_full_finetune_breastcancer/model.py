...
class PyTorch_Transformers_ZoeVN_sam_full_finetune_breastcancer(PyTorchAbstractClass):
    def __init__(self, config=None):
        self.config = config if config else dict()
        device = self.config.pop("_device", "cpu")
        multi_gpu = self.config.pop("_multi_gpu", False)
        model_id = "ZoeVN/sam_full_finetune_breastcancer"
    ...
    def postprocess(self, model_output):
        ...
        for output, original_size in zip(model_output, self.original_sizes):
            # original_size is (width, height), interpolate needs (height, width)
