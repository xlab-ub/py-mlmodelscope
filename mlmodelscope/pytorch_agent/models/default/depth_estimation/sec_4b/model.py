...
class PyTorch_Transformers_OpenIXCLab_SeC_4B(PyTorchAbstractClass):
    def __init__(self, config=None):
        self.config = config if config else dict()
        device = self.config.pop("_device", "cpu")
        multi_gpu = self.config.pop("_multi_gpu", False)

        model_id = "OpenIXCLab/SeC-4B"
    ...
    def preprocess(self, input_images):
        images = [Image.open(img_path) for img_path in input_images]
    ...
    def postprocess(self, model_output):
        ...
        for output, original_size in zip(model_output, self.original_sizes):
            ...
            prediction = torch.nn.functional.interpolate(
                ...
                size=(original_size[1], original_size[0]), # PIL size is (width, height)
                mode="bicubic",
                align_corners=False,
            )
