...
class PyTorch_Transformers_Kitooo_Sidewalk_Seg_Base(PyTorchAbstractClass):
    def __init__(self, config=None):
        self.config = config if config else dict()
        device = self.config.pop("_device", "cpu")
        multi_gpu = self.config.pop("_multi_gpu", False)

        self.image_processor = AutoImageProcessor.from_pretrained("kitooo/sidewalk-seg-base")
        
        if multi_gpu and device == "cuda":
            self.model = AutoModelForDepthEstimation.from_pretrained("kitooo/sidewalk-seg-base", device_map="auto", torch_dtype="auto")
        else:
            self.model = AutoModelForDepthEstimation.from_pretrained("kitooo/sidewalk-seg-base")
    ...
    def postprocess(self, model_output):
        ...
        for output, original_size in zip(model_output, self.original_sizes):
            # original_size is in (width, height) format, interpolate needs (height, width)
