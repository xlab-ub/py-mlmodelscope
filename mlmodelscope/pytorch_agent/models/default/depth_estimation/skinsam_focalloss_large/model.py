...
class PyTorch_Transformers_Ahishamm_Skinsam_Focalloss_Large(PyTorchAbstractClass):
    def __init__(self, config=None):
        self.config = config if config else dict()
        device = self.config.pop("_device", "cpu")
        multi_gpu = self.config.pop("_multi_gpu", False)
        model_id = "ahishamm/skinsam_focalloss_large"
    ...
    def postprocess(self, model_output):
        ...
        for output, original_size in zip(model_output, self.original_sizes):
            # original_size is (width, height), interpolate expects (height, width)
