...
class PyTorch_Transformers_Sharka_CIVQA_LayoutLMv2(PyTorchAbstractClass):
    def __init__(self, config=None):
        self.config = config if config else dict()
        device = self.config.pop("_device", "cpu")
        multi_gpu = self.config.pop("_multi_gpu", False)

        model_id = "Sharka/CIVQA_LayoutLMv2"
    ...
    def preprocess(self, input_document_images_and_questions):
        ...
        self.encoding = self.processor(images, questions, return_tensors="pt", padding=True, truncation=True)
