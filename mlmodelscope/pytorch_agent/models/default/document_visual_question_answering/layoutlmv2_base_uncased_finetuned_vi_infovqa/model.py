...
class PyTorch_Transformers_LayoutLMv2_Base_Uncased_Finetuned_Vi_InfoVQA(PyTorchAbstractClass):
    def __init__(self, config=None):
        self.config = config if config else dict()
        device = self.config.pop("_device", "cpu")
        multi_gpu = self.config.pop("_multi_gpu", False)

        model_id = "tiennvcs/layoutlmv2-base-uncased-finetuned-vi-infovqa"
    ...
    def postprocess(self, model_output):
        ...
        for i in range(len(start_logits)):
            ...
            if end_index >= start_index:
                answer_ids = self.last_input_ids[i][start_index : end_index + 1]
