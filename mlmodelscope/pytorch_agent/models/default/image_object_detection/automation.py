...
def image_object_detection_model_automation(models_to_add=None):
...
"""
    ...
    class ModelConfig(BaseModel):
        """Configuration details for a PyTorch object detection model."""

        imports: str = Field(
            description="The complete imports section. Include torch, torchvision detection models or transformers, PIL Image, warnings if needed. Use proper formatting with newlines.",
            default="import torch\nfrom torchvision.models.detection import fasterrcnn_resnet50_fpn, FasterRCNN_ResNet50_FPN_Weights\nfrom PIL import Image\nimport warnings",
        )

        class_name: str = Field(
            description="The class name for the model. Should be descriptive and follow PascalCase convention, e.g., 'PyTorch_TorchVision_FasterRCNN_ResNet50_FPN'.",
        )
        ...
        postprocess_body: str = Field(
            description="The complete body of the postprocess method. Should extract scores, labels, boxes from predictions and convert to list format. Return tuple of lists: [scores], [labels], [boxes]. Include proper indentation (8 spaces).",
        )
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                f"""
You are an expert in PyTorch object detection models. Your task is to generate a complete, working model.py file.

**IMPORTANT GUIDELINES:**

1. **Model Type Detection:**
   - TorchVision models (Faster R-CNN, RetinaNet, etc.): Use torchvision.models.detection
   - Transformers models (DETR, YOLOS, etc.): Use transformers library
   - Most common: Faster R-CNN variants
...
6. **Postprocess Method:**
   - Extract predictions: model_output[0]['scores'], model_output[0]['labels'], model_output[0]['boxes']
   - Convert to lists: .tolist()
   - Return as tuple: [scores], [labels], [boxes]
...
""",
    ...
    BASE_DIR = "mlmodelscope/pytorch_agent/models/default/image_object_detection"
            ...
            while not os.path.exists(model_py_path) or (
                ...
                filled_template = MODEL_TEMPLATE.format(
                    ...
                    class_name=model_config_dict.get(
                        "class_name", "PyTorch_Object_Detection_Model"
                    ),
...
if __name__ == "__main__":
    image_object_detection_model_automation(models_to_add=["facebook/detr-resnet-50"])
