...
class PyTorch_Transformers_ConditionalDetr50SignatureDetector(PyTorchAbstractClass):
    def __init__(self, config=None):
        self.config = config if config else dict()
        device = self.config.pop("_device", "cpu")
        multi_gpu = self.config.pop("_multi_gpu", False)
        model_id = "tech4humans/conditional-detr-50-signature-detector"
    ...
    def postprocess(self, model_output):
        results = self.processor.post_process_object_detection(model_output, target_sizes=self.target_sizes, threshold=0.5)[0]
