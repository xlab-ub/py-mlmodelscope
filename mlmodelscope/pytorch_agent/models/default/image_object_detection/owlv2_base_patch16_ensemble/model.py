...
class PyTorch_Transformers_Xenova_OwlV2_Base_Patch16_Ensemble(PyTorchAbstractClass):
    def __init__(self, config=None):
        self.config = config if config else dict()
        device = self.config.pop("_device", "cpu")
        multi_gpu = self.config.pop("_multi_gpu", False)
        self.candidate_labels = self.config.pop("candidate_labels", ["a photo of a cat", "a photo of a dog"])
    ...
    def preprocess(self, input_images):
        ...
        model_input = self.processor(text=self.candidate_labels, images=images, return_tensors="pt")
        return model_input
