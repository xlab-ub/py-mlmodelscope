...
def question_answering_model_automation(models_to_add=None):
...
"""
    ...
    class ModelConfig(BaseModel):
        """Configuration details for a PyTorch question answering model."""

        imports: str = Field(
            description="The complete imports section. Include transformers (AutoTokenizer, AutoModelForQuestionAnswering). Use proper formatting with newlines.",
            default="from transformers import AutoTokenizer, AutoModelForQuestionAnswering",
        )

        class_name: str = Field(
            description="The class name for the model. Should be descriptive and follow PascalCase convention, e.g., 'PyTorch_Transformers_BERT_QA'.",
        )
        ...
        preprocess_body: str = Field(
            description="The complete body of the preprocess method. Should tokenize question-context pairs with return_tensors='pt', padding=True. Include proper indentation (8 spaces).",
        )

        predict_body: str = Field(
            description="The complete body of the predict method. Should call model and return start_logits and end_logits. Include proper indentation (8 spaces).",
        )

        postprocess_body: str = Field(
            description="The complete body of the postprocess method. Should extract answer from logits using argmax, decode tokens to get answer text. Include proper indentation (8 spaces).",
        )
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                f"""
You are an expert in PyTorch question answering models. Your task is to generate a complete, working model.py file.

**IMPORTANT GUIDELINES:**

1. **Model Type:**
   - Most QA models use AutoModelForQuestionAnswering (BERT, RoBERTa, etc.)
...
2. **Imports:**
   ...
   - Standard: `from transformers import AutoTokenizer, AutoModelForQuestionAnswering`
...
4. **Preprocess Method:**
   ...
   - Tokenize with return_tensors="pt", padding=True

5. **Predict Method:**
   - Call model: `return self.model(**model_input)`

6. **Postprocess Method:**
   - Get start/end positions with argmax
            ...
            while not os.path.exists(model_py_path) or (
                ...
                filled_template = MODEL_TEMPLATE.format(
                    ...
                    class_name=model_config_dict.get(
                        "class_name", "PyTorch_Question_Answering_Model"
                    ),
