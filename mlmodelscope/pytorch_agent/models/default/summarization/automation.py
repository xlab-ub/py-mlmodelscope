...
def summarization_model_automation(models_to_add=None):
...
"""
    ...
    class ModelConfig(BaseModel):
        """Configuration details for a PyTorch summarization model."""

        imports: str = Field(
            description="The complete imports section. Include transformers (AutoTokenizer, AutoModelForSeq2SeqLM). Use proper formatting with newlines.",
            default="from transformers import AutoTokenizer, AutoModelForSeq2SeqLM",
        )

        class_name: str = Field(
            description="The class name for the model. Should be descriptive and follow PascalCase convention, e.g., 'PyTorch_Transformers_BART_Large_CNN'.",
        )
        ...
        preprocess_body: str = Field(
            description="The complete body of the preprocess method. Should tokenize input texts with return_tensors='pt', padding=True, truncation=True. Include proper indentation (8 spaces).",
        )
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                f"""
You are an expert in PyTorch summarization models. Your task is to generate a complete, working model.py file.

**IMPORTANT GUIDELINES:**

1. **Model Type:**
   - BART models: Use AutoModelForSeq2SeqLM
   - T5 models: Use AutoModelForSeq2SeqLM
   - Pegasus models: Use PegasusForConditionalGeneration
   - Most summarization models are sequence-to-sequence
...
4. **Preprocess Method:**
   - Tokenize with truncation: `return self.tokenizer(input_texts, return_tensors="pt", padding=True, truncation=True)`
            ...
            while not os.path.exists(model_py_path) or (
                ...
                filled_template = MODEL_TEMPLATE.format(
                    ...
                    class_name=model_config_dict.get(
                        "class_name", "PyTorch_Summarization_Model"
                    ),
