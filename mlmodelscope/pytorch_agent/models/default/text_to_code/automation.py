...
def text_to_code_model_automation(models_to_add=None):
...
"""
    ...
    class ModelConfig(BaseModel):
        """Configuration details for a PyTorch text-to-code model."""

        imports: str = Field(
            description="The complete imports section. Include transformers classes (AutoTokenizer, AutoModelForCausalLM). Use proper formatting with newlines.",
            default="from transformers import AutoTokenizer, AutoModelForCausalLM",
        )

        class_name: str = Field(
            description="The class name for the model. Should be descriptive and follow PascalCase convention, e.g., 'PyTorch_Transformers_CodeGen_350M_Mono'.",
        )
        ...
        preprocess_body: str = Field(
            description="The complete body of the preprocess method. Should use tokenizer with return_tensors='pt', padding=True. Include proper indentation (8 spaces).",
        )

        predict_body: str = Field(
            description="The complete body of the predict method. Should call model.generate with **model_input, pad_token_id, and max_new_tokens. Include proper indentation (8 spaces).",
        )

        postprocess_body: str = Field(
            description="The complete body of the postprocess method. Should use tokenizer.batch_decode with skip_special_tokens=True. Include proper indentation (8 spaces).",
        )
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                f"""
You are an expert in PyTorch text-to-code models. Your task is to generate a complete, working model.py file.

**IMPORTANT GUIDELINES:**

1. **Model Type Detection:**
   - CodeGen models: Use AutoModelForCausalLM
   - Code LLMs (StarCoder, CodeLlama, etc.): Use AutoModelForCausalLM
   - All follow causal LM pattern with left-side padding
...
5. **Predict Method:**
   - Generate: `return self.model.generate(**model_input, pad_token_id=self.tokenizer.eos_token_id, max_new_tokens=self.max_new_tokens)`
            ...
            while not os.path.exists(model_py_path) or (
                ...
                filled_template = MODEL_TEMPLATE.format(
                    ...
                    class_name=model_config_dict.get(
                        "class_name", "PyTorch_Text_To_Code_Model"
                    ),
