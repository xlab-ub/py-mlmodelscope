...
class PyTorch_Transformers_GPT_OSS_20B(PyTorchAbstractClass):
    def __init__(self, config=None):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            "openai/gpt-oss-20b", padding_side="left"
        )
        ...
        self.max_new_tokens = (
            self.config["max_new_tokens"] if "max_new_tokens" in self.config else 32
        )
    ...
    def preprocess_chat(self, input_texts):
        ...
        encoded = self.tokenizer.apply_chat_template(
            formatted_messages, return_dict=True, return_tensors="pt"
        )
    ...
    def postprocess_chat(self, model_output):
        ...
        tokens = torch.tensor([200006, 173781, 200005, 17196, 200008]).to(
            response.device
        )
