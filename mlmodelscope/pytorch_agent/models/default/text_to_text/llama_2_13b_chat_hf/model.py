...
class PyTorch_Transformers_Llama_2_13B_Chat_HF(PyTorchAbstractClass):
  def __init__(self, config=None):
    ...
    self.max_nex_tokens = self.config['max_new_tokens'] if 'max_new_tokens' in self.config else 32 
  
  def preprocess(self, input_texts):
    return self.tokenizer(input_texts, return_tensors="pt", padding=True).input_ids 

  def predict(self, model_input): 
    return self.model.generate(model_input, max_new_tokens=self.max_nex_tokens) 
