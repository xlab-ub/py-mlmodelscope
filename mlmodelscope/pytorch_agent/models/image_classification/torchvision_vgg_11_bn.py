...
class TorchVision_VGG_11_BN:
  def __init__(self):
    self.model = torch.hub.load('pytorch/vision:v0.10.0', 'vgg11_bn', pretrained=True)
    self.model.eval()

    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 
    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/pytorch/torchvision/vgg/TorchVision_VGG_11_BN.yml 
