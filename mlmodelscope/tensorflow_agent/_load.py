from .models.tensorflow_abc import TensorFlowAbstractClass 
import inspect
import ast
import pathlib
import sys
...
def create_instance_from_model_manifest_file(task, model_name, security_check=True, config=None, user='default'):
    ...
    # Find the class that inherits from TensorFlowAbstractClass
    abstract_class = TensorFlowAbstractClass
    ...
    if target_class:
        ...
        raise ModuleNotFoundError("No subclass of TensorFlowAbstractClass was found in the module.")
...
def _load(task, model_name, security_check=True, config=None, user='default'):
    ...
    tensorflow_abstract_class_instance = create_instance_from_model_manifest_file(task, model_name, security_check, config, user) 
