...
class TensorFlow_Transformers_Wav2Vec2_Base_960h(TensorFlowAbstractClass):
  def __init__(self, config=None):
    self.config = config if config else {} 
    self.processor = Wav2Vec2Processor.from_pretrained("facebook/wav2vec2-base-960h") 
    self.model = TFWav2Vec2ForCTC.from_pretrained("facebook/wav2vec2-base-960h") 
  ...
  def preprocess(self, input_audios):
    ...
    model_input = self.processor(input_audios, sampling_rate=self.sampling_rate, return_tensors="tf", padding="longest") 
    return model_input 
  ...
  def postprocess(self, model_output):
    predicted_ids = tf.argmax(model_output, axis=-1) 
