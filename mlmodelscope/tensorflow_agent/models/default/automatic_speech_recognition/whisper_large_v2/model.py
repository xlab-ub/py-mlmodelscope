...
class TensorFlow_Transformers_Whisper_Large_v2(TensorFlowAbstractClass):
  def __init__(self, config=None):
    self.config = config if config else {} 
    self.processor = WhisperProcessor.from_pretrained("openai/whisper-large-v2") 
    self.model = TFWhisperForConditionalGeneration.from_pretrained("openai/whisper-large-v2") 
