...
class TensorFlow_Transformers_Whisper_Medium_EN(TensorFlowAbstractClass):
  def __init__(self, config=None):
    self.config = config if config else {} 
    self.processor = WhisperProcessor.from_pretrained("openai/whisper-medium.en") 
    self.model = TFWhisperForConditionalGeneration.from_pretrained("openai/whisper-medium.en") 
