...
class TensorFlow_AI_Matrix_DenseNet_121:
  def __init__(self):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/tensorflow/ai_matrix/AI_Matrix_DenseNet_121.yml 
    self.inLayer = 'Images' 
    self.outLayer = 'logits' 

    self.inNode = None 
    self.outNode = None 

    model_file_url = "https://zenodo.org/record/3269944/files/Densenet121-NHWC.pb" 
...
def init():
  return TensorFlow_AI_Matrix_DenseNet_121()
