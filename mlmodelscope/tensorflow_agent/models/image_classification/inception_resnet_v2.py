...
class TensorFlow_Inception_ResNet_v2:
  def __init__(self):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/tensorflow/inception/Inception_ResNet_v2.yml 
    self.inLayer = 'input' 
    self.outLayer = 'InceptionResnetV2/Logits/Predictions' 

    self.inNode = None 
    self.outNode = None 

    model_file_url = "http://s3.amazonaws.com/store.carml.org/models/tensorflow/graphs/inception_resnet_v2_frozen.pb" 
  ...
  def preprocess(self, input_images):
    for i in range(len(input_images)):
      input_images[i] = self.preprocess_image(cv2.imread(input_images[i]), [299, 299, 3], False) 
...
def init():
  return TensorFlow_Inception_ResNet_v2() 
