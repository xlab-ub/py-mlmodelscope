...
class TensorFlow_Inception_v3:
  def __init__(self):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/tensorflow/inception/Inception_v3.yml 
    self.inLayer = 'input' 
    self.outLayer = 'InceptionV3/Predictions/Reshape_1' 

    self.inNode = None 
    self.outNode = None 

    model_file_url = "http://s3.amazonaws.com/store.carml.org/models/tensorflow/graphs/inception_v3_frozen.pb" 
...
def init():
  return TensorFlow_Inception_v3() 
