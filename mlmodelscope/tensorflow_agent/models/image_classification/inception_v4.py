...
class TensorFlow_Inception_v4:
  def __init__(self):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/tensorflow/inception/Inception_v4.yml 
    self.inLayer = 'input' 
    self.outLayer = 'InceptionV4/Logits/Predictions' 

    self.inNode = None 
    self.outNode = None 

    model_file_url = "http://s3.amazonaws.com/store.carml.org/models/tensorflow/graphs/inception_v4_frozen.pb" 
...
def init():
  return TensorFlow_Inception_v4() 
