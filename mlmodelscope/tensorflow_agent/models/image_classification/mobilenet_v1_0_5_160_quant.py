...
class TensorFlow_MobileNet_v1_0_5_160_Quant:
  def __init__(self):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/tensorflow/mobilenet/MobileNet_v1_0.5_160_Quant.yml 
    self.inLayer = 'input' 
    ...
    model_file_name = 'mobilenet_v1_0.5_160_quant_frozen.pb' 
    model_path = os.path.join(temp_path, model_file_name) 

    if not os.path.exists(model_path): 
      model_file_url = "https://s3.amazonaws.com/store.carml.org/models/tensorflow/models/mobilenet_v1_2018_08_02/mobilenet_v1_0.5_160_quant.tgz" 
...
def init():
  return TensorFlow_MobileNet_v1_0_5_160_Quant()
