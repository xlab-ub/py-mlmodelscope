...
class TensorFlow_MobileNet_v1_1_0_224:
  def __init__(self):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/tensorflow/mobilenet/MobileNet_v1_1.0_224.yml 
    self.inLayer = 'input' 
    ...
    # the model file name is same with one from mlcommons_mobilenet_v1.py 
    model_file_name = 'mobilenet_v1_1.0_224_frozen.pb' 
    model_path = os.path.join(temp_path, 'mobilenet_v1_2018_08_02_' + model_file_name) 

    if not os.path.exists(model_path): 
      model_file_url = "https://s3.amazonaws.com/store.carml.org/models/tensorflow/models/mobilenet_v1_2018_08_02/mobilenet_v1_1.0_224.tgz" 
      tgz_file_name = '_'.join(model_file_url.split('/')[-2:]) 
      # https://www.tensorflow.org/api_docs/python/tf/keras/utils/get_file 
      tf.keras.utils.get_file(fname=tgz_file_name, origin=model_file_url, cache_subdir='.', cache_dir=temp_path) 

      with tarfile.open(os.path.join(temp_path, tgz_file_name)) as tgz_file: 
        # https://docs.python.org/3/library/tarfile.html 
        with  tgz_file.extractfile('./' + model_file_name) as m_f: 
          with open(model_path, 'wb') as wb_f: 
            wb_f.writelines(m_f.readlines()) 
...
def init():
  return TensorFlow_MobileNet_v1_1_0_224()
