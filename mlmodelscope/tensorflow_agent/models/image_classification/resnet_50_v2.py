...
class TensorFlow_ResNet_50_V2:
  def __init__(self):
    temp_path = os.path.join(pathlib.Path(__file__).resolve().parent.parent.parent, 'tmp') 
    if not os.path.isdir(temp_path): 
      os.mkdir(temp_path) 

    # https://github.com/c3sr/dlmodel/blob/master/models_demo/vision/image_classification/tensorflow/resnet/ResNet_50_v2.yml 
    self.inLayer = 'input' 
    self.outLayer = 'resnet_v2_50/predictions/Reshape_1' 

    self.inNode = None 
    self.outNode = None 

    model_file_url = "https://s3.amazonaws.com/store.carml.org/models/tensorflow/graphs/resnet_v2_50_frozen.pb" 
...
def init():
  return TensorFlow_ResNet_50_V2()
