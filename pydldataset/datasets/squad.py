...
def read_squad_examples(input_file, is_training, count, version_2_with_negative=False):
...
class SQUAD: 
  ...
  def __init__(self, count):
    ...
    tokenizer = BertTokenizer(os.path.join(data_dir, "vocab.txt"))
    print("Reading examples...")
    eval_examples = read_squad_examples(input_file=os.path.join(data_dir, "dev-v1.1.json"),
      is_training=False, count=count, version_2_with_negative=False)
    ...
    convert_examples_to_features(
      ...
      verbose_logging=False)
    # if count <= 0 or count > len(self.eval_features):
    #   count = len(self.eval_features)
    # return count
