...
class Visual_URL_Data(DatasetAbstractClass):
    def __init__(self, inputs): 
        self.root = self.get_directory('url_data') 
        print("Visual URL DATA")
        # self.inputs = inputs 
        self.inputs = []
        ...
        try:
            ...
            for i in range(len(self.inputs[idx])):
                ...
                if self.inputs[idx][i]["inputType"] in ["IMAGE","AUDIO","VIDEO","DOCUMENT"]:
                    response = requests.get(self.inputs[idx][i]["src"]) 
                # Check if the request was successful (status code 200)
                    if response.status_code == 200:
                        url_file_name = self.inputs[idx][i]["src"].split('/')[-1] 
                    ...
                    object_to_be_returned.append(self.inputs[idx][i]["src"])
                else:
