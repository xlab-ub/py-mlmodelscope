...
def parse_set_argument(arg_properties):
    ...
    if parser.parse_known_args()[0].standalone == 'true': 
        for arg in arg_properties:
          parser.add_argument(arg['optional_keyword'], type=arg['type'], nargs='?' , help=arg['help'], choices=arg['choices'] if 'chocies' in arg else None, default=arg['default'] if 'default' in arg else None)
    else: 
...
def connect_db_mq(args, agent):
    ...
    cur = conn.cursor()
    return mq_name, mq_host, mq_port, mq_user, mq_password

def set_configuration(args, agent):
  user          = args.user 
  task          = args.task 
...
def callback(properties, body, agent):
...
def connect_to_rabbitmq_server(mq_user, mq_password, mq_host, mq_port, mq_name, callback):
