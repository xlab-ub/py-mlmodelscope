...
#   parser.add_argument("--agent", type=str, nargs='?', default="tensorflow", choices=["pytorch", "tensorflow", "onnxruntime", "mxnet", "jax"], help="Which framework to use") 
...
#     parser.add_argument("--task", type=str, nargs='?', default="image_semantic_segmentation", help="The name of the task to predict.") 
#     parser.add_argument("--model_name", type=str, nargs='?', default="deeplabv3_mobilenet_v2_dm_05_pascal_voc_train_aug", help="The name of the model") 
...
#     parser.add_argument("--architecture", type=str, nargs='?', default="cpu", choices=["cpu", "gpu"], help="Which Processing Unit to use") 
...
from run_common_module import parse_set_argument, connect_db_mq, connect_to_rabbitmq_server, callback, set_configuration

args = parse_set_argument([
  {'optional_keyword': '--user', 'type': str, 'default': 'default', 'help': 'The name of the user'},
  {'optional_keyword': '--task', 'type': str, 'default': 'image_semantic_segmentation', 'help': 'The name of the task to predict.'},
  {'optional_keyword': '--model_name', 'type': str, 'default': 'deeplabv3_mobilenet_v2_dm_05_pascal_voc_train_aug', 'help': "The name of the model"},
  {'optional_keyword': '--config_file', 'type': str, 'default': 'false', 'choices': ["false", "true"], 'help': "Whether to use config file (.json)"},
  {'optional_keyword': '--config_file_path', 'type': str, 'default': 'config.json', 'help': 'The path of the config file'},
  {'optional_keyword': '--architecture', 'type': str, 'default': 'cpu', 'choices': ["cpu", "gpu"], 'help': "Which Processing Unit to use"},
  {'optional_keyword': '--num_warmup', 'type': int, 'default': 2, 'help': 'Total number of warmup steps for predict.'},
  {'optional_keyword': '--dataset_name', 'type': str, 'default': "test_data", 'help': 'The name of the dataset for predict.'},
  {'optional_keyword': '--batch_size', 'type': int, 'default': 1, 'help': 'Total batch size for predict.'},
  {'optional_keyword': '--gpu_trace', 'type': str, 'default': 'false', 'choices': ["false", "true"], 'help': "Whether to trace GPU activities"},
  {'optional_keyword': '--detailed_result', 'type': str, 'default': 'false', 'choices': ["false", "true"], 'help': "Whether to get detailed result"},
  {'optional_keyword': '--security_check', 'type': str, 'default': 'false', 'choices': ["false", "true"], 'help': "Whether to perform security check on the model file"},
])

agent = args.agent

if args.standalone == 'true': 
  set_configuration(args, agent)
else:
  mq_name, mq_host, mq_port, mq_user, mq_password = connect_db_mq(args=args, agent=agent)
  connect_to_rabbitmq_server(mq_name=mq_name, mq_host=mq_host, mq_port= mq_port, mq_password=mq_password, mq_user=mq_user, callback=callback)